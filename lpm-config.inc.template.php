...
// Сколько видео разрешено сжимать одновременно на всём сервере.
...
define('VIDEO_COMPRESS_MAX_PARALLEL', 2);
// Предельное время сжатия одного файла, секунды. По его истечении процесс
// прерывается, сжатие считается неудачным (остаётся оригинал), а слот
// освобождается для следующего файла из очереди.
