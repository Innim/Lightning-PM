...
// Секретный токен для вызовов GitLab Hook - общий и для системного хука
// инстанса, и для хуков репозиториев (в GitLab это поле Secret Token).
// Обязателен: пока он не задан, вызовы хука отклоняются.
define('GITLAB_HOOK_TOKEN', '');

// Заводить ли в репозитории хук на события пайплайнов (Pipeline events)
// автоматически, при создании ветки задачи. К системному хуку инстанса,
// который приносит влития и пуши, флаг отношения не имеет: тот всегда
// заводится вручную.
//
// Включайте только на боевом стенде: адрес хука берется из SITE_URL, а
// репозитории у стендов общие с боевыми - включенный флаг на дев-стенде
// перепишет в них адрес хука на свой. Локальный адрес (localhost и подобные)
// таск распознает сам и настраивать хук откажется, но адрес дев-стенда,
// выставленный наружу, от боевого не отличить - там защиты нет.
// define('GITLAB_AUTO_SETUP_WEBHOOKS', true);
...
// Пока список пуст, адрес пользователя считается неизвестным, и ограничение
// попыток входа по одному адресу не применяется вовсе (см. страницу «Статус»):
// за прокси REMOTE_ADDR у всех запросов один, и такое ограничение закрыло бы
// вход сразу всем.
// Писать сюда надо адрес, с которого запрос приходит к веб-серверу приложения.
// Если приложение работает в Docker, это не адрес самого nginx и не 127.0.0.1,
// а адрес шлюза docker-сети — он зависит от установки, поэтому значения
// по умолчанию тут нет. Посмотреть его можно на странице «Статус».
// define('SECURITY_TRUSTED_PROXIES', ['172.17.0.1']);
