...
- AI-generated artifacts: each one is built by a `*Builder` that owns the gates, the context sent to the model, the prompt and a `PROMPT_VERSION`. Generation happens ONLY on an explicit user action (never on page render), failures are never persisted, and token usage is logged per call to `LPMLog::CH_AI`. Two kinds exist, and the caching rules below apply only to the first:
  - **Cached** — the artifact is stored and shown again on later page loads (the issue summary, `lpm-core/modules/ai/summary/`).
  - **One-shot draft** — the model's output is handed to the user to edit and becomes something else entirely (the test checklist, `lpm-core/modules/ai/checklist/`, whose draft the user edits and publishes as a comment). Nothing is stored, so there is no `sourceHash` and nothing to invalidate; the builder additionally converts the response to the target format (`IssueTestChecklistBuilder::toCommentText()`).
