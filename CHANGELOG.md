...
### Added
- Быстрый переход к задаче по номеру в проекте: компактное поле в верхнем подменю на страницах проекта (список задач, завершенные, Scrum, просмотр задачи). Введите номер и нажмите Enter/→, чтобы открыть задачу.
