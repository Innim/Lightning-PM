...
{	
	...
	{
		if( !preg_match( "/^[0-9]{1,2}:[0-9]{2}$/", $time ) ) return false;
		else return true;
	}
	...
	{
		if( !$this->checkTime( $timeStart ) || !$this->checkTime( $timeEnd ) ) return false;
	...
	{
		if( !$this->checkDate( $dateStart ) || !$this->checkDate( $dateEnd ) ) return false;
	...
	{
		if( !$this->checkTime( $time ) ) return $this->error( 'Неверный формат даты' );
	...
	{
		if (!preg_match( "/^[0-9]{2}:[0-9]{2}:[0-9]{2}$/", $dbTime )) return false;
	...
	{
		if (!$this->checkDate( $date )) return false;
		$days = (int)$days;
