...
class Worker extends User
{
    ...
    */
        ...
        switch ($var) {
            ...
            case 'leavingTime': {
                if (count($this->_records) > 0) {
                    if ($this->_records[0]->$var == 0) {
                        return '';
                    } else {
                        return $this->_records[0]->{'get' . ucfirst($var)}();
                    }
                } else {
                    return false;
                }
    ...
    public function addRecord(WSRecord $record)
    {
        ...
        $delta = 0;
        if ($record->lunchBreak) {
            $delta += self::LUNCH_BREAK;
        }
        ...
        // считаем часы
        if ($record->leavingTime == 0 && $record->isToday()) {
            $leavingTime = DateTimeUtils::date() - DateTimeUtils::dayBegin();
        } else {
