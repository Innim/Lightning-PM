...
class WorkStudyPage extends BasePage
{
    ...
    public function init()
    {
        ...
        if (isset($_POST['tickWorker'], $_POST['come'],
            ...
            && is_array($_POST['come'])) {
            if (!isset($_POST['lunchBreak'])) {
                $_POST['lunchBreak'] = false;
            }
    ...
    public function getWorkers($date = '')
    {
        if (!$this->_workers) {
            ...
            if (!$prepare = $this->_db->preparet($sql, LPMTables::WORK_STUDY)) {
                return false;
            }
    ...
    private function tickWorker(
        ...
        if ($query = $this->_db->queryt($sql, LPMTables::WORKERS)) {
            if ($query->num_rows < 1) {
                return $this->error('Нет такого сотрудника');
            } else {
        ...
        if (!$this->_db->queryt($sql, LPMTables::WORK_STUDY)) {
            return $this->error();
        }
