...
/**
 * Сколько раз пытаться занять номер задачи в проекте при её создании.
 * Номер выдаётся как «последний плюс один», и одновременное создание
 * задач в одном проекте может выбрать один и тот же номер — тогда попытка
 * повторяется с новым номером.
 * @var int
 */
define('ISSUE_CREATE_MAX_ATTEMPTS', 10);
...
/**
 * Максимальная длина поискового запроса по задачам - запрос длиннее обрезается,
 * чтобы в выборку по всем задачам не уходила строка произвольного размера.
 * @var int
 */
define('ISSUE_SEARCH_MAX_LENGTH', 255);

/**
 * Максимальная длина контекста предметной области проекта для ИИ.
 * Контекст уходит в каждый запрос к модели и оплачивается токенами,
 * поэтому предел жёсткий.
 * @var int
 */
define('AI_PROJECT_CONTEXT_MAX_LENGTH', 2000);

/**
 * Максимальная длина шаблона описания задачи и правил её оформления.
 * Правила уходят в каждый запрос к модели и оплачиваются токенами,
 * но в отличие от контекста проекта это правила на всё приложение,
 * поэтому предел мягче.
 * @var int
 */
define('ISSUE_GUIDELINES_MAX_LENGTH', 10000);
...
/**
 * Адреса обратных прокси, чьему заголовку `X-Forwarded-For` можно верить.
 *
 * Пустой список означает, что адрес пользователя установить нельзя: заголовок
 * присылает кто угодно, а `REMOTE_ADDR` за прокси один на все запросы.
 * Ограничения, которые бьют по всем владельцам адреса сразу, при пустом списке
 * не применяются - см. {@see ClientIp} и страницу «Статус».
 *
 * Значения по умолчанию нет: адрес прокси зависит от установки.
 * @var string[]
 */
if (!defined('SECURITY_TRUSTED_PROXIES')) {
    define('SECURITY_TRUSTED_PROXIES', []);
}
