...
class WorkStudyService extends LPMBaseService
{
	...
	public function addWorker( $userId, $hours, $comingTime ) {		
		...
		if (!$row = $query->fetch_assoc()) return $this->error( 'Нет такого пользователя' );
		if (!empty( $row['id'] )) 
		...
		if (!$this->_db->queryt( $sql, LPMTables::WORKERS )) return $this->errorDBSave();
	...
	public function beforeFilter( $calledFunc ) {
		if (!parent::beforeFilter( $calledFunc )) return false;
