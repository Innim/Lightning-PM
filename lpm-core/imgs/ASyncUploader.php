...
class ASyncUploader
{
    ...
    public function init( $params ) {
        ...
        if (!$this->_user->isAdmin()) return $this->error( 'Недостаточно прав' );
        ...
        switch ($params['type'])
        {
            case 'preview' : {
                ...
                if (!$news) return $this->error( 'Не найдено новости с таким идентификатором' );
                
                $uploader = new BOImgUpload( 1, false,
                                             array( News::PREVIEW_WIDTH, News::PREVIEW_HEIGHT ), 
                                             News::PREVIEWS_DIR, News::PREVIEWS_PREFIX );
                if ($uploader->isErrorsExist()) {
                    $errors = $uploader->getErrors();
                    return $this->error( $errors[0] );
                }
                
                if ($uploader->getLoadedCount() == 0) return $this->error( 'Файлы не загружены' );
                ...
                if (!$this->_db->queryt( $sql, BOTables::NEWS )) {
                	if ($uploader) $uploader->removeImgs();
                	return $this->saveInDBError();
                } else {
                    // удаляем старое изображение если оно было
                    if ($news->previewImg) $news->previewImg->removeAll();
                }                
            } break;
			case 'photo' : {
                $newsId = (float)$_REQUEST['id'];
                ...
                if (!$this->_db->queryt( $sql, BOTables::NEWS )) {
                    ...
                    // удаляем старое изображение если оно было
                    if ($news->photoImg) $news->photoImg->removeAll();
                }                
            } break;
    ...
    private function answer() {
        ...
        if (!$this->_success) $answer['error'] = $this->_error;
