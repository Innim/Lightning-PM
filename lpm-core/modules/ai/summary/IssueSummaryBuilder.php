...
class IssueSummaryBuilder
{
     ...
     *
     * Одного названия модели не хватает, чтобы понять, где задача находится
     * и завершена ли она.
     */
...
- текущее состояние задачи описывай по её статусу, а не по отметкам в комментариях:
  статус учитывает их все и отменяет устаревшие;
...
TEXT;
     ...
     * поле задачи, попадающее в запрос, смена статуса или его уточнения
     * (в том числе перемещение стикера по доске и отметка о взятии
     * в тестирование), добавление, удаление или смена типа комментария,
     * переименование проекта, правка контекста проекта для ИИ. Изменения,
     * которых модель не видит (например, приоритет), сводку не обесценивают.
     *
     * @param Issue $issue Задача.
     * @param array<Comment> $comments Все комментарии задачи.
    ...
    private static function buildIssueBlock(Issue $issue)
    {
        ...
        // У задачи в тесте результат тестирования уже назван в статусе — отдельно
